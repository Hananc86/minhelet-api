declare module "graphql-sequelize" {
    ...
    import { GraphQLScalarType, GraphQLResolveInfo, GraphQLInterfaceType, GraphQLType, GraphQLEnumType, GraphQLFieldConfig, GraphQLFieldConfigMap, GraphQLFieldResolver } from "graphql";

    export type IBeforeFn<TContext> = <T>(options: T, args, context: TContext, info: GraphQLResolveInfo) => T | Promise<T>;
    export type IAfterFn<TContext> = <T>(result: T, args, context: TContext, info: GraphQLResolveInfo) => T | Promise<T>;

    interface IMapType {
        [k: string]: string;
    }

    export type InstanceOf<T> = T extends Sequelize.Model<infer U, any> ? U : never
